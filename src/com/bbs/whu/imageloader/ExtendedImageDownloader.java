package com.bbs.whu.imageloader;
...
public class ExtendedImageDownloader extends URLConnectionImageDownloader {
	...
	private InputStream getStreamFromAssets(URI imageUri) throws IOException {
		String filePath = imageUri.toString().substring(
				PROTOCOL_ASSETS_PREFIX.length()); // Remove "assets://" prefix
													// from image URI
		return context.getAssets().open(filePath);
	}

	private InputStream getStreamFromDrawable(URI imageUri) {
		String drawableIdString = imageUri.toString().substring(
				PROTOCOL_DRAWABLE_PREFIX.length()); // Remove "drawable://"
													// prefix from image URI
